...
const DisplayPost = (props) => {
    ...
    const singlePost = async (source) => {
        try {
            const res = await Axios.get(`https://blogigy-backend-api.onrender.com/api/posts/post-details/${postid}`, {
    ...
    const handleComment = async (event) => {
        ...
        try {
            const res = await Axios.post(`https://blogigy-backend-api.onrender.com/api/posts/${postid}/comments`, JSON.stringify({ commentBody }), {
                headers: {
    ...
    const handleCommentDelete = async (postid, commentid) => {
        try {
            const res = await Axios.delete(`https://blogigy-backend-api.onrender.com/api/posts/${postid}/comments/${commentid}`, {
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('token')
                }
            });
    ...
    const handleUpvotes = async () => {
        ...
        if (!loggedUser) {
            ...
            try {
                const res = await Axios.put(`https://blogigy-backend-api.onrender.com/api/posts/${postid}/upvotes`, JSON.stringify(loggedUser._id), {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + localStorage.getItem('token')
                    }
    ...
    const handleDownvotes = async () => {
        ...
        if (!loggedUser) {
            ...
            try {
                const res = await Axios.put(`https://blogigy-backend-api.onrender.com/api/posts/${postid}/downvotes`, JSON.stringify(loggedUser._id), {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + localStorage.getItem('token')
                    }
                            ...
                            <div key={index} className="mb-2">
                                ...
                                <span className="text-secondary"><em>{formatDate(comment.createdAt)}</em></span>
                                {
                                    ...
                                    <Fragment>
                                        ...
                                        <EditComment setState={setCommentState} state={commentState}  singlePost={singlePost}  cmtid={comment._id} comment={comment} postid={postid}/>
                                        <i 
